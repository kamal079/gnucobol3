...
$@{envvar:default@} or $@{envvar:-default@}.
@*
...
Note: If you want to *slightly* speed up a program's startup time, remove all
     @code{of the comments from the actual real configuration file that is processed}
@*
@*
@verbatim
...
Environment name:  COB_TRACE_FORMAT
  Parameter name:  trace_format
         ...
         Example:  TRACE_FORMAT  "Line: %L %S"
            Note:  format of GC2.2 and older: "PROGRAM-ID: %I 	Line: %L 	%S"
...
kamal079 - added mf format (start)
...
kamal079 - added mf file format (end)
...
Environment name:  COB_DISPLAY_PUNCH_FILE
  Parameter name:  display_punch_file
         Purpose:  Defines file to be created on first DISPLAY UPON SYSPUNCH/SYSPCH
